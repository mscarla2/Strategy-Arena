...
def main():
    ...
    all_dates.sort()
    print(f"Found {len(all_dates)} historical sessions.")
    ...
    print(f"Loading bars for {len(unique_entries)} unique tickers...")
    
    bars_map = {}
    # Use ThreadPool for parallel bar loading (cached mostly)
    ...
    print(f"Loaded bars for {len(bars_map)}/{len(unique_entries)} tickers.")
    
    # Run simulation per day
    all_trades = []
    
    print("\nSimulating across all dates...")
    for sim_date in all_dates:
        ...
        # Run simulation for this day (require_support_ok=True for "After" logic)
        # The user wants "our runtoday on all with the monte carlo results".
        # We should run with require_support_ok=True to show the robust logic!
        day_trades = simulate_all(
            day_unique, day_bars,
            profit_target_pct=3.75,
            stop_loss_pct=1.0,
            max_entry_attempts=10,
            require_support_ok=False,
            verbose=False,
        )
        all_trades.extend(day_trades)
        print(f"  {sim_date}: {len(day_trades)} trades.")
        
    if not all_trades:
        print("No trades simulated across all dates.")
        return
        
    print(f"\nTotal Trades Simulated across all dates: {len(all_trades)}")
    ...
    for _ in range(n_iterations):
        ...
        for t in all_trades:
            ...
            # Random slippage (0.1% to 0.5% per leg)
    ...
    print("\n=== Global Monte Carlo Results (1,000 Iterations) ===")
